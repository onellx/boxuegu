...
public class RegisterActivity extends AppCompatActivity {
        ...
        btn_register.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if(TextUtils.isEmpty(userName)){
                    ...
                    Toast.makeText(RegisterActivity.this,"请输入再次输入密码",Toast.LENGTH_SHORT).show();
                    return;
                    ...
                    boolean falg=saveRegisterInfo(userName,psw);
                    if(!falg){
    ...
    private boolean  isExitUserName(String userName){
        ...
        UserBean userBean=JsonUtil.jsonToPojo(result,UserBean.class);
...
//        String spPsw=sp.getString("userName","");
    ...
    private boolean saveRegisterInfo(String userName,String psw){
        String md5Psw= MD5Utils.md5(psw);
        UserBean userBean=new UserBean();
        userBean.setUsername(userName);
        userBean.setPasswd(md5Psw);
        String result=OkHttpUtil.postSyncRequest(HttpApi.REGISTER,JsonUtil.objectToJson(userBean));
        result=JsonUtil.jsonToPojo(result,String.class);
        if(result.equals("true")){
            SharedPreferences sp=getSharedPreferences("loginInfo",MODE_PRIVATE);
            SharedPreferences.Editor editor=sp.edit();
            //以用户名为key密码为value保存在SharePreferences
