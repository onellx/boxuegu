...
import com.boxuegu.activity.adapter.VideoListAdapter;
        ...
        try {
            ...
            for (int i = 0; i < jsonArray.length(); i++) {
                ...
                if (jsonObj.getInt("chapterId") == chapterId) {
                    bean.setChapterId(jsonObj.getInt("chapterId"));
                    bean.setVideoId(Integer.parseInt(jsonObj.getString("videoId")));
                    bean.setTitle(jsonObj.getString("title"));
                    bean.setSecondTitle(jsonObj.getString("secondTitle"));
                    bean.setVideoPath(jsonObj.getString("videoPath"));
                    videoList.add(bean);
                }
                bean = null;
            }
